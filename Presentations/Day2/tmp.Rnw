...
\begin{frame}[fragile]
	\frametitle{Vector subsetting}
	If we want to 1th, 3th and 2th elements of the vector animals\\
	\centering Try it out
	\pause
	\rule{\textwidth}{0.4pt}
<<>>=
...
%%%%%%% 
\begin{frame}[fragile]
		\frametitle{Vector conditional subsetting}
...
\begin{frame}[fragile]
	\frametitle{Vector conditional subsetting}
	If we want all the elements bigger than 50.\\
		\rule{\textwidth}{0.4pt}
<<>>=
weight_g > 50
@  
